...
const EXPECTED = process.env.EXPECTED_VERSION || '2.7.0-alpha.23';
...
async function json(path, timeoutMs = CATALOG_TIMEOUT_MS) {
  ...
  const response = await fetchWithTimeout(`${BASE}${path}${separator}a23=${Date.now()}-${Math.random()}`, {
...
function isKnownGenericPoster(value) {
  const text = String(value || '');
  return /(?:fallback|placeholder|default|studio-catalog)/i.test(text) && !isInternalStudioPoster(text);
}
function posterHost(value) { try { return new URL(String(value || '')).hostname.toLowerCase(); } catch { return ''; } }
function posterQualitySummary(metas = []) {
  const posters = metas.map(item => String(item.poster || '')).filter(Boolean);
  const unique = new Set(posters).size;
  const generated = posters.filter(isInternalStudioPoster).length;
  const generic = posters.filter(isKnownGenericPoster).length;
  const external = posters.filter(value => /^https:\/\//i.test(value) && !isInternalStudioPoster(value) && !isKnownGenericPoster(value)).length;
  const hosts = new Set(posters.map(posterHost).filter(Boolean)).size;
  return { cards: metas.length, unique, generated, generic, external, hosts };
}
...
(async () => {
  ...
  console.log('\n=== WEAK-CATALOG POSTER QUALITY ===');
  const qualityTargets = Object.freeze({
    'tpb4k.studio.digitalplayground.top': { minExternal: 10, minUnique: 30 },
    'tpb4k.studio.onlyfans.top': { minExternal: 10, minUnique: 35 },
    'tpb4k.studio.xvideosred.top': { minExternal: 8, minUnique: 25 },
  });
  for (const [catalogId, target] of Object.entries(qualityTargets)) {
    const metas = rounds[1].get(catalogId)?.metas || [];
    const quality = posterQualitySummary(metas);
    if (quality.unique < Math.min(target.minUnique, quality.cards)) {
      issue(catalogId, `only ${quality.unique}/${quality.cards} unique posters; expected ${Math.min(target.minUnique, quality.cards)}`);
    }
    if (quality.generic > 0) issue(catalogId, `${quality.generic} legacy generic studio posters remain`);
    if (quality.external < Math.min(target.minExternal, quality.cards)) {
      issue(catalogId, `only ${quality.external}/${quality.cards} verified external metadata posters`);
    }
    console.log(`POSTERS ${catalogId}: cards=${quality.cards} unique=${quality.unique} external=${quality.external} generated=${quality.generated} generic=${quality.generic} hosts=${quality.hosts}`);
  }
        ...
        for (const item of metas.slice(0, 10)) {
          try {
            ...
            if (contentType.includes('image/svg+xml')) {
              ...
              if (!text.includes('<svg') || text.length < 500) throw new Error('invalid internal SVG poster');
            } else {
  ...
  console.log('\nSUCCESS: Alpha.23 complete live acceptance passed.');
})().catch(error => {
