#!/usr/bin/env python3
"""Persistent curl_cffi Chrome transport dedicated to public Pornhub pages.

The process reads one JSON object per line from stdin and writes one JSON
response per line to stdout. It keeps Pornhub cookies and anti-bot state in a
provider-isolated session. Only public catalogue, video-page, and media-API
requests on approved Pornhub hosts are allowed.
"""
...
def validate_url(value: Any) -> str:
    ...
    if parsed.scheme != "https" or host not in ALLOWED_HOSTS:
        raise ValueError("URL host is not approved for Pornhub Chrome transport")
...
def handle(message: dict[str, Any]) -> dict[str, Any]:
    ...
    max_bytes = max(
        1024,
        min(int(message.get("maxBytes", 8 * 1024 * 1024)), MAX_BYTES_HARD_LIMIT),
    )
    method = str(message.get("method") or "GET").upper()
    if method not in {"GET", "POST", "HEAD"}:
        raise ValueError("Chrome transport method is not allowed")
