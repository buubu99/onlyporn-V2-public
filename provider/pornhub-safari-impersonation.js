...
class PornhubChromeClient {
  ...
  start() {
    ...
    child.stderr.on('data', chunk => {
      ...
      if (message) {
        logger.warn(
          { helper: 'pornhub-chrome', error: message },
          'Pornhub Chrome helper stderr'
        );
      }
    });
    ...
    child.on('exit', (code, signal) => {
      this.onExit(
        child,
        new Error(`Pornhub Chrome helper exited (${code ?? 'null'}/${signal ?? 'none'})`)
      );
    });
  }
  ...
  onLine(line) {
    ...
    try {
      ...
      logger.warn({ helper: 'pornhub-chrome' }, 'Pornhub Chrome helper returned invalid JSON');
      return;
    }
  ...
  async fetchText(url, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Pornhub Chrome helper timed out after ${timeoutMs}ms`));
  ...
  async fetchJson(url, options = {}) {
    ...
    try {
      ...
      const invalid = new Error(`Pornhub Chrome helper returned invalid JSON: ${error.message}`);
...
module.exports = new PornhubChromeClient();
