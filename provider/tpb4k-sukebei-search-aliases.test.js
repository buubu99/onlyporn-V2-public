...
test('Sukebei uncensored aliases overlap two-request waves and preserve completed responses under one 27-second budget', () => {
  const fs = require('node:fs');
  const source = fs.readFileSync(require.resolve('./tpb4k'), 'utf8');
  ...
  assert.match(method, /aliasOverallBudgetMs = 27_000/);
  assert.match(method, /aliasUsefulGraceMs = 500/);
  assert.match(method, /aliasWaveStaggerMs = 1_500/);
  assert.match(method, /const tasks = queries\.map\(\(search, index\) => \(async \(\) =>/);
  assert.match(method, /Math\.floor\(index \/ 2\)/);
  assert.match(method, /Promise\.allSettled\(tasks\)/);
  assert.match(method, /firstUsefulPromise\.then\(\(\) => 'useful'\)/);
  assert.match(method, /responses\.push\(\.\.\.aliasResponses\.filter\(Boolean\)\)/);
  ...
  assert.doesNotMatch(method, /__onlypornAliasTimedOut/);
