...
test('Phase 2B status exposes source names but never endpoint URLs or API credentials', () => {
  ...
  assert.deepEqual(status.configuredDiscoverySources, ['hentai', 'pornrips', 'studio-metadata', 'sukebei', 'sukebeiHentai', 'torrent-index', 'yesporn']);
...
test('built-in adapter registry covers every Phase 2 source while preserving the Stripchat gate', () => {
  ...
  assert.deepEqual(listAdapters(), ['hentai', 'platform-hybrid', 'pornrips', 'stripchat', 'studio-metadata', 'sukebei', 'sukebei-hentai', 'torrent-index', 'tpdb', 'yesporn']);
  assert.equal(installed.phaseGates.stripchat, 7);
  assert.deepEqual(installed.configuredDiscoverySources, ['hentai', 'platform-hybrid', 'pornrips', 'sukebei', 'sukebei-hentai', 'torrent-index', 'yesporn']);
});
...
test('all 27 OnlyPorn catalog IDs remain unique and unified-resolution', () => {
  assert.equal(tpb4kCatalogs.length, 27);
  assert.equal(new Set(tpb4kCatalogs.map(item => item.id)).size, 27);
...
test('Phase 2B release wiring retains the TPB torrent-index adapter', () => {
  assert.match(require('../package.json').version, /^2\.7\.0-alpha\.(?:2[5-9]|[3-9]\d|\d{3,})$/);
  assert.equal(require('../package.json').scripts['test:release'], 'npm test');
});
