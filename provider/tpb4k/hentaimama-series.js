...
function dooplayAjaxEndpoint(html) {
  const source = normalizeEmbeddedMarkup(html);
  const match = source.match(/(?:ajaxurl|admin_ajax|ajax_url|url)\s*[:=]\s*(?:"([^"]*admin-ajax\.php[^"]*)"|'([^']*admin-ajax\.php[^']*)'|`([^`]*admin-ajax\.php[^`]*)`)/i)
    || source.match(/(?:https?:)?\/\/[^\s"'<>]+\/wp-admin\/admin-ajax\.php[^\s"'<>]*/i);
  const raw = match?.[1] || match?.[2] || match?.[3] || match?.[0] || `${ORIGIN}/wp-admin/admin-ajax.php`;
  const url = safePublicUrl(raw, ORIGIN);
  try { return url && new URL(url).origin === ORIGIN ? url : `${ORIGIN}/wp-admin/admin-ajax.php`; }
  catch { return `${ORIGIN}/wp-admin/admin-ajax.php`; }
}

function dooplayPlayerOptions(html) {
  const source = normalizeEmbeddedMarkup(html);
  const output = [];
  const positions = new Map();
  const add = value => {
    const post = String(value?.post || '').match(/^\d{1,12}$/)?.[0] || '';
    const nume = String(value?.nume || '').match(/^\d{1,4}$/)?.[0] || '1';
    const type = compact(value?.type || 'tv').toLowerCase();
    if (!post || !/^(?:tv|movie|episode)$/.test(type)) return;
    const normalizedType = type === 'episode' ? 'tv' : type;
    const nonce = compact(value?.nonce || '');
    const key = `${post}:${nume}:${normalizedType}`;
    const record = Object.freeze({ post, nume, type: normalizedType, nonce });
    ...
    if (!output[existing].nonce && nonce) output[existing] = record;
  };
  for (const match of source.matchAll(/<[^>]+(?:data-post|data-nume|data-type|data-option)[^>]*>/gi)) {
    const tag = match[0];
    add({
      post: htmlAttribute(tag, 'data-post') || htmlAttribute(tag, 'data-id'),
      nume: htmlAttribute(tag, 'data-nume') || htmlAttribute(tag, 'data-option') || htmlAttribute(tag, 'data-index'),
      type: htmlAttribute(tag, 'data-type') || 'tv',
      nonce: htmlAttribute(tag, 'data-nonce') || htmlAttribute(tag, 'data-security'),
    });
  }
  const scriptSource = source.replace(/<[^>]+>/g, ' ');
  for (const match of scriptSource.matchAll(/(?:post|id)\s*[:=]\s*['"]?(\d{1,12})['"]?[\s\S]{0,300}?nume\s*[:=]\s*['"]?(\d{1,4})['"]?[\s\S]{0,200}?type\s*[:=]\s*['"]?(tv|movie|episode)/gi)) {
    add({ post: match[1], nume: match[2], type: match[3] });
  }
  const fallbackPost = source.match(POST_ID_RE)?.[1] || '';
  if (!output.length && fallbackPost) {
    for (const nume of ['1', '2', '3']) add({ post: fallbackPost, nume, type: 'tv' });
  }
  return Object.freeze(output.slice(0, 12));
}
...
function createHentaiMamaSeriesAdapter(options = {}) {
  ...
  async function resolveEpisode(request) {
    const stages = { series: 0, episodePage: 0, inlineMedia: 0, ajaxPostId: 0, dooplayOptions: 0, dooplayRequests: 0, ajaxFields: 0, ajaxIframes: 0, ajaxMedia: 0, playerPages: 0, playerFetched: 0, playerDepth: 0, nestedPlayers: 0, playerMedia: 0, validated: 0, errors: [] };
    ...
    const dooplayOptions = dooplayPlayerOptions(html);
    stages.dooplayOptions = dooplayOptions.length;
    const postId = html.match(POST_ID_RE)?.[1] || dooplayOptions[0]?.post || '';
    if (postId) stages.ajaxPostId = 1;
    const ajaxEndpoint = dooplayAjaxEndpoint(html);
    const ajaxRequests = [];
    const requestKeys = new Set();
    const rememberAjaxRequest = fields => {
      const key = new URLSearchParams(fields).toString();
      if (!requestKeys.has(key)) { requestKeys.add(key); ajaxRequests.push(fields); }
    };
    for (const option of dooplayOptions) {
      const base = { action: 'doo_player_ajax', post: option.post, nume: option.nume, type: option.type };
      rememberAjaxRequest(base);
      if (option.nonce) {
        rememberAjaxRequest({ ...base, nonce: option.nonce });
        rememberAjaxRequest({ ...base, _wpnonce: option.nonce });
      }
    }
    if (postId) {
      rememberAjaxRequest({ action: 'get_player_contents', a: postId });
      rememberAjaxRequest({ action: 'get_player_contents', post_id: postId });
      rememberAjaxRequest({ action: 'get_player_contents', id: postId });
      rememberAjaxRequest({ action: 'get_player_contents', post: postId });
    }
    for (const fields of ajaxRequests.slice(0, 18)) {
      try {
        const body = new URLSearchParams(fields).toString();
        const response = await safeFetch(ajaxEndpoint, {
          fetchImpl: options.fetchImpl,
          checkDns: options.checkDns,
          timeoutMs: options.config.requestTimeoutMs,
          method: 'POST',
        ...
        stages.dooplayRequests += fields.action === 'doo_player_ajax' ? 1 : 0;
