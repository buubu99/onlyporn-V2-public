...
test('Phase 1 defines the exact 28 selected TPB4K board catalogs', () => {
...
test('TPB4K stays disabled by default and exposes its 28 descriptors only behind the feature flag', () => {
...
test('candidate ranking prefers validated direct and cached 4K results and deduplicates hashes', () => {
  ...
  assert.ok(candidateScore(direct) > candidateScore(cached4k));
  assert.ok(candidateScore(cached4k) > candidateScore(uncached));
  const results = sortCandidates(dedupeCandidates([duplicate, uncached, direct, cached4k]));
  assert.equal(results.length, 3);
  assert.equal(results[0].kind, 'direct-hls');
  assert.equal(results[1].infoHash, HASH_4K);
  assert.equal(results[1].seeders, 10);
});
...
test('provider foundation normalizes catalog, meta, and stream results without leaking HTML pages', async () => {
  ...
  assert.equal(meta.meta.extra.tpb4k.source, 'pornrips');

  const result = await provider.handleStream({ type: 'movie', id: catalog.metas[0].id });
  assert.equal(result.streams.length, 2);
  assert.equal(result.streams[0].url, 'https://media.example/scene/master.m3u8');
  assert.equal(result.streams[1].infoHash, HASH_4K);
