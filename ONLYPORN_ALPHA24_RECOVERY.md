...
- Prioritizes SexMex scenes that already contain multiple genuine hashes, instead of burying them below one-hash queued scenes.
- Versions the persistent catalogue cache key so a previous Render release cannot overwrite a new poster, identity, or failover policy after deployment.
